...
async def test_llm_connection(payload: dict):
    ...
    try:
        ...
        # Check if this is a Bedrock endpoint
        is_bedrock = "bedrock" in base_url.lower() or "amazonaws.com" in base_url.lower()
        ...
        if is_bedrock:
            ...
            try:
                ...
                if "bedrock://" in base_url:
                    parsed_url = base_url.replace("bedrock://", "")
                    if parsed_url and parsed_url.strip():
                        region = parsed_url.strip().split("/")[0]
                ...
                test_messages = [
                    {
                        "role": "user",
                        "content": [{"text": "Say 'Connection test successful' if you can read this."}]
                    }
                ]
                ...
                if "output" in response and "message" in response["output"]:
                    ...
                    if "content" in message and len(message["content"]) > 0:
                        ...
                        if content:
                            logger.info(f"Bedrock test successful for model {model} in region {region}")
                            return {
                                "success": True,
                                "message": f"Bedrock connection successful! Model {model} responded correctly.",
                                "response": content
                            }
                
                return {"success": False, "message": "Bedrock responded but with unexpected format"}
                ...
                if error_code == "AccessDeniedException":
                    return {"success": False, "message": f"Access denied. Please check your API key and model access permissions: {error_message}"}
                elif error_code == "ValidationException":
                    return {"success": False, "message": f"Invalid model ID or configuration: {error_message}"}
                elif error_code == "ResourceNotFoundException":
                    return {"success": False, "message": f"Model '{model}' not found in region {region}: {error_message}"}
                elif error_code == "ThrottlingException":
                    return {"success": False, "message": "Rate limit exceeded. Please try again later."}
                else:
                    return {"success": False, "message": f"Bedrock API error [{error_code}]: {error_message}"}
                    
            except ImportError:
                return {"success": False, "message": "boto3 is not installed. Please install it to use Bedrock: pip install boto3>=1.40.0"}
            except Exception as e:
                logger.error(f"Bedrock test failed: {e}", exc_info=True)
                return {"success": False, "message": f"Bedrock connection test failed: {str(e)}"}
